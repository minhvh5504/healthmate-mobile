...
import '../../providers/add_prescription/add_prescription_provider.dart';
import 'widgets/add_prescription_date_field.dart';
import 'widgets/add_prescription_header.dart';
import 'widgets/add_prescription_image_picker.dart';
import 'widgets/add_prescription_text_field.dart';

class AddPrescriptionPage extends ConsumerStatefulWidget {
  const AddPrescriptionPage({super.key, this.prescription});

  final Prescription? prescription;

  @override
  ConsumerState<AddPrescriptionPage> createState() =>
      _AddPrescriptionPageState();
}

class _AddPrescriptionPageState extends ConsumerState<AddPrescriptionPage>
  ...
  void initState() {
    ...
    final initialState = AddPrescriptionState.initial(widget.prescription);
  ...
  Future<void> _showImageSourceSheet(AddPrescriptionNotifier notifier) async {
    _unfocusInputs();
  ...
  Future<void> _selectDate({
    required AddPrescriptionNotifier notifier,
  ...
  Widget build(BuildContext context) {
    final provider = addPrescriptionProvider(widget.prescription);
    final state = ref.watch(provider);
    final notifier = ref.read(provider.notifier);

    ref.listen<AddPrescriptionState>(provider, (previous, next) {
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        AddPrescriptionImagePicker(
                          image: state.image,
                          onChangeTap: () => _showImageSourceSheet(notifier),
                          onViewImage: state.image != null
                              ? () =>
                                    _showLargeImage(context, file: state.image)
                              : null,
                        ),
                        SizedBox(height: 18.h),
                        AddPrescriptionTextField(
                        ...
                        Row(
                          children: [
                            Expanded(
                              child: AddPrescriptionDateField(
  ...
  void _showLargeImage(BuildContext context, {File? file}) {
    if (file == null) return;
